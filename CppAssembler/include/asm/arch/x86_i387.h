...
namespace CppAsm::X86
{
	...
	protected:
		...
		static constexpr void write_Ext_Prefix(BLOCK& block) {
			block.template pushRaw<byteOrder, uint8_t>(0xD9);
		}

		template<class BLOCK>
		static constexpr void write_Unmask_Exception_Prefix(BLOCK& block) {
			block.template pushRaw<byteOrder, uint8_t>(0x9B);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void template_1operand_float(BLOCK& block, const detail::OpcodeLarge& opcode, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void template_1operand_float80(BLOCK& block, const detail::OpcodeLarge& opcode, const detail::OpcodeLarge& opcode80, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void template_1operand_float(BLOCK& block, const detail::OpcodeLarge& opcode, RegFPU reg) {
		...
		static constexpr void template_1operand_mem_uncheck(BLOCK& block, const detail::OpcodeLarge& opcode, const Mem32<MODE>& mem) {
			mem.writeSegmPrefix(block);
			common::write_Opcode(block, opcode.getOpcode());
			mem.write(block, opcode.getMode());
		}
		...
		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void template_1operand_int(BLOCK& block, const detail::OpcodeLarge& opcode, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void template_1operand_int64(BLOCK& block, const detail::OpcodeLarge& opcode, const detail::OpcodeLarge& opcode64, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void template_Extended(BLOCK& block, common::Opcode opcode) {
			write_Ext_Prefix(block);
			common::write_Opcode(block, opcode);
		}

		template<class BLOCK>
		static constexpr void template_2Byte_Opcode(BLOCK& block, common::Opcode opcodeFirst, common::Opcode opcodeSecond) {
...
#pragma region FPU operations
		template<class BLOCK>
		static constexpr void F2xm1(BLOCK& block) {
			template_Extended(block, 0xF0);
		}

		template<class BLOCK>
		static constexpr void Fabs(BLOCK& block) {
			template_Extended(block, 0xE1);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fadd(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fadd(BLOCK& block, TopRegFPU, RegFPU reg) {
			template_1operand_float(block, detail::OpcodeLarge(0xD8, 0b000), reg);
		}

		template<class BLOCK>
		static constexpr void Fadd(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDC, 0b000), reg);
		}

		template<class BLOCK>
		static constexpr void Faddp(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDE, 0b000), reg);
		}

		template<class BLOCK>
		static constexpr void Faddp(BLOCK& block) {
			Faddp(block, ST1, ST);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fiadd(BLOCK& block, const Mem32<MODE>& mem) {
		...
		static constexpr void Fbld(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE = TBYTE_PTR, AddressMode MODE, class BLOCK>
		static constexpr void Fbstp(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fchs(BLOCK& block) {
			template_Extended(block, 0xE0);
		}

		template<class BLOCK>
		static constexpr void Fnclex(BLOCK& block) {
			template_2Byte_Opcode(block, 0xDB, 0xE2);
		}

		template<class BLOCK>
		static constexpr void Fclex(BLOCK& block) {
			write_Unmask_Exception_Prefix(block);
			Fnclex(block);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fcom(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fcom(BLOCK& block, RegFPU reg = ST1) {
			template_1operand_float(block, detail::OpcodeLarge(0xD8, 0b010), reg);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fcomp(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fcomp(BLOCK& block, RegFPU reg = ST1) {
			template_1operand_float(block, detail::OpcodeLarge(0xD8, 0b011), reg);
		}

		template<class BLOCK>
		static constexpr void Fcompp(BLOCK& block) {
			template_2Byte_Opcode(block, 0xDE, 0xD9);
		}

		template<class BLOCK>
		static constexpr void Fcos(BLOCK& block) {
			template_Extended(block, 0xFF);
		}
		
		template<class BLOCK>
		static constexpr void Fdecstp(BLOCK& block) {
			template_Extended(block, 0xF6);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fdiv(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fdiv(BLOCK& block, TopRegFPU, RegFPU reg) {
			template_1operand_float(block, detail::OpcodeLarge(0xD8, 0b110), reg);
		}

		template<class BLOCK>
		static constexpr void Fdiv(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDC, 0b111), reg);
		}

		template<class BLOCK>
		static constexpr void Fdivp(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDE, 0b111), reg);
		}

		template<class BLOCK>
		static constexpr void Fdivp(BLOCK& block) {
			Fdivp(block, ST1, ST);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fidiv(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fdivr(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fdivr(BLOCK& block, TopRegFPU, RegFPU reg) {
			template_1operand_float(block, detail::OpcodeLarge(0xD8, 0b111), reg);
		}

		template<class BLOCK>
		static constexpr void Fdivr(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDC, 0b110), reg);
		}

		template<class BLOCK>
		static constexpr void Fdivrp(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDE, 0b110), reg);
		}

		template<class BLOCK>
		static constexpr void Fdivrp(BLOCK& block) {
			Fdivrp(block, ST1, ST);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fidivr(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Ffree(BLOCK& block, RegFPU reg) {
			template_1operand_float(block, detail::OpcodeLarge(0xDD, 0b000), reg);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Ficom(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Ficomp(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fild(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fincstp(BLOCK& block) {
			template_Extended(block, 0xF7);
		}

		template<class BLOCK>
		static constexpr void Fninit(BLOCK& block) {
			template_2Byte_Opcode(block, 0xDB, 0xE3);
		}

		template<class BLOCK>
		static constexpr void Finit(BLOCK& block) {
			write_Unmask_Exception_Prefix(block);
			Fninit(block);
		}
		...
		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fist(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fistp(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fisttp(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fld(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fmul(BLOCK& block, TopRegFPU, RegFPU reg) {
			template_1operand_float(block, detail::OpcodeLarge(0xD8, 0b001), reg);
		}

		template<class BLOCK>
		static constexpr void Fmul(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDC, 0b001), reg);
		}

		template<class BLOCK>
		static constexpr void Fmulp(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDE, 0b001), reg);
		}

		template<class BLOCK>
		static constexpr void Fmulp(BLOCK& block) {
			Fmulp(block, ST1, ST);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fimul(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fnop(BLOCK& block) {
			template_Extended(block, 0xD0);
		}

		template<class BLOCK>
		static constexpr void Fpatan(BLOCK& block) {
			template_Extended(block, 0xF3);
		}

		template<class BLOCK>
		static constexpr void Fprem(BLOCK& block) {
			template_Extended(block, 0xF8);
		}

		template<class BLOCK>
		static constexpr void Fprem1(BLOCK& block) {
			template_Extended(block, 0xF5);
		}

		template<class BLOCK>
		static constexpr void Fptan(BLOCK& block) {
			template_Extended(block, 0xF2);
		}

		template<class BLOCK>
		static constexpr void Frndint(BLOCK& block) {
			template_Extended(block, 0xFC);
		}

		template<AddressMode MODE, class BLOCK>
		static constexpr void Frstor(BLOCK& block, const Mem32<MODE>& mem) {
			template_1operand_mem_uncheck(block, detail::OpcodeLarge(0xDD, 0b100), mem);
		}

		template<AddressMode MODE, class BLOCK>
		static constexpr void Fsave(BLOCK& block, const Mem32<MODE>& mem) {
			write_Unmask_Exception_Prefix(block);
			Fnsave(block, mem);
		}

		template<AddressMode MODE, class BLOCK>
		static constexpr void Fnsave(BLOCK& block, const Mem32<MODE>& mem) {
			template_1operand_mem_uncheck(block, detail::OpcodeLarge(0xDD, 0b110), mem);
		}

		template<class BLOCK>
		static constexpr void Fscale(BLOCK& block) {
			template_Extended(block, 0xFD);
		}

		template<class BLOCK>
		static constexpr void Fsin(BLOCK& block) {
			template_Extended(block, 0xFE);
		}

		template<class BLOCK>
		static constexpr void Fsincos(BLOCK& block) {
			template_Extended(block, 0xFB);
		}

		template<class BLOCK>
		static constexpr void Fsqrt(BLOCK& block) {
			template_Extended(block, 0xFA);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fst(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fst(BLOCK& block, RegFPU reg) {
			template_1operand_float(block, detail::OpcodeLarge(0xDD, 0b010), reg);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fstp(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fstp(BLOCK& block, RegFPU reg) {
			template_1operand_float(block, detail::OpcodeLarge(0xDD, 0b011), reg);
		}

		template<MemSize SIZE = WORD_PTR, AddressMode MODE, class BLOCK>
		static constexpr void Fstcw(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE = WORD_PTR, AddressMode MODE, class BLOCK>
		static constexpr void Fnstcw(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<AddressMode MODE, class BLOCK>
		static constexpr void Fstenv(BLOCK& block, const Mem32<MODE>& mem) {
			write_Unmask_Exception_Prefix(block);
			Fnstenv(block, mem);
		}

		template<AddressMode MODE, class BLOCK>
		static constexpr void Fnstenv(BLOCK& block, const Mem32<MODE>& mem) {
			template_1operand_mem_uncheck(block, detail::OpcodeLarge(0xD9, 0b110), mem);
		}

		template<MemSize SIZE = WORD_PTR, AddressMode MODE, class BLOCK>
		static constexpr void Fstsw(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE = WORD_PTR, AddressMode MODE, class BLOCK>
		static constexpr void Fnstsw(BLOCK& block, const Mem32<MODE>& mem) {
		...
		static constexpr void Fstsw(BLOCK& block) {
		...
		template<Reg16 REG = AX, class BLOCK>
		static constexpr void Fnstsw(BLOCK& block) {
			static_assert(REG == AX, "Fnstsw: Invalid register operand");
			template_2Byte_Opcode(block, 0xDF, 0xE0);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fsub(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fsub(BLOCK& block, TopRegFPU, RegFPU reg) {
			template_1operand_float(block, detail::OpcodeLarge(0xD8, 0b100), reg);
		}

		template<class BLOCK>
		static constexpr void Fsub(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDC, 0b101), reg);
		}

		template<class BLOCK>
		static constexpr void Fsubp(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDE, 0b101), reg);
		}

		template<class BLOCK>
		static constexpr void Fsubp(BLOCK& block) {
			Fsubp(block, ST1, ST);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fisub(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fsubr(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Fsubr(BLOCK& block, TopRegFPU, RegFPU reg) {
			template_1operand_float(block, detail::OpcodeLarge(0xD8, 0b101), reg);
		}

		template<class BLOCK>
		static constexpr void Fsubr(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDC, 0b100), reg);
		}

		template<class BLOCK>
		static constexpr void Fsubrp(BLOCK& block, RegFPU reg, TopRegFPU) {
			template_1operand_float(block, detail::OpcodeLarge(0xDE, 0b100), reg);
		}

		template<class BLOCK>
		static constexpr void Fsubrp(BLOCK& block) {
			Fsubrp(block, ST1, ST);
		}

		template<MemSize SIZE, AddressMode MODE, class BLOCK>
		static constexpr void Fisubr(BLOCK& block, const Mem32<MODE>& mem) {
		...
		template<class BLOCK>
		static constexpr void Ftst(BLOCK& block) {
			template_Extended(block, 0xE4);
		}

		template<class BLOCK>
		static constexpr void Fucom(BLOCK& block, RegFPU reg = ST1) {
			template_1operand_float(block, detail::OpcodeLarge(0xDD, 0b100), reg);
		}

		template<class BLOCK>
		static constexpr void Fucomp(BLOCK& block, RegFPU reg = ST1) {
			template_1operand_float(block, detail::OpcodeLarge(0xDD, 0b101), reg);
		}

		template<class BLOCK>
		static constexpr void Fucompp(BLOCK& block) {
			template_2Byte_Opcode(block, 0xDA, 0xE9);
		}

		template<class BLOCK>
		static constexpr void Fxam(BLOCK& block) {
			template_Extended(block, 0xE5);
		}

		template<class BLOCK>
		static constexpr void Fxch(BLOCK& block, RegFPU reg = ST1) {
			template_1operand_float(block, detail::OpcodeLarge(0xD9, 0b001), reg);
		}

		template<class BLOCK>
		static constexpr void Fxtract(BLOCK& block) {
			template_Extended(block, 0xE4);
		}

		template<class BLOCK>
		static constexpr void FyL2X(BLOCK& block) {
			template_Extended(block, 0xF1);
		}

		template<class BLOCK>
		static constexpr void FyL2XP1(BLOCK& block) {
			template_Extended(block, 0xF9);
		}

		/* FPU wait */
		template<class BLOCK>
		static constexpr void Fwait(BLOCK& block) {
			Wait(block);
		}

		/* FPU wait */
		template<class BLOCK>
		static constexpr void Wait(BLOCK& block) {
